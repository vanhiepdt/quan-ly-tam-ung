...
export default async function GiaoDichPage() {
  ...
  let donVi: Array<{ id: string; ten: string; lan_cuoi?: string }> = []
  ...
  try {
    ...
    const [nguoiLayHdResult, donViResult, cauHinhGiay] = await Promise.all([
      ...
      // Lấy đơn vị với logic ưu tiên: chưa tiếp trong năm này, hoặc tiếp xa nhất
      db.query<{ id: string; ten: string; lan_cuoi: string | null }>(`
        select dv.id, dv.ten, to_char(max(gd.ngay), 'DD/MM/YYYY') as lan_cuoi
        from don_vi dv
        left join giao_dich gd on gd.don_vi_id = dv.id and not gd.da_xoa and extract(year from gd.ngay) = extract(year from now())
        where dv.dang_hoat_dong
        group by dv.id, dv.ten
        order by
          case when max(gd.ngay) is null then 0 else 1 end,  -- chưa tiếp trong năm lên đầu
          max(gd.ngay) asc nulls first,  -- tiếp xa nhất lên trước
          dv.ten
      `),
      docCauHinhGiay(),
    ])
    collectors = nguoiLayHdResult.rows.map(r => ({ id: r.id, ten: r.ten, taiKhoan: taiKhoanTu(r) }))
    donVi = donViResult.rows.map(r => ({ id: r.id, ten: r.ten, lan_cuoi: r.lan_cuoi || undefined }))
